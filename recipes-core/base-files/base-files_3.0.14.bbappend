...
G40_BSP_VERSION = "\r Rootfs Version : iW-PRGQZ-SC-01-R2.0-REL0.2-YoctoZeus\n"
G40_HOST = "iWave-G40M"
...
do_install () {
	...
	# deal with hostname
	if [ -n "${G40_HOST}" ]; then
               echo ${G40_HOST} > ${D}${sysconfdir}/hostname
        fi
}

do_install_basefilesissue () {
	install -m 644 ${WORKDIR}/issue*  ${D}${sysconfdir}
	if [ -n "${G40_BSP_VERSION}" ]; then
               printf "${G40_BSP_VERSION} " >> ${D}${sysconfdir}/issue
               printf "${G40_BSP_VERSION} " >> ${D}${sysconfdir}/issue.net
        fi
        if [ -n "${G40_HOST}" ]; then
               echo ${G40_HOST} > ${D}${sysconfdir}/hostname
        fi
